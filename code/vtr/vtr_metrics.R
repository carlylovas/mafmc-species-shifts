...
# Load data ----
vtr_dat <- read_csv(here("data","processed","vessel_trip_reports.csv"))
...
decade_bins <- vtr_dat |> 
  ...
  group_by(species_name, decade) |>
  summarise(`5%` = Hmisc::wtd.quantile(lat,  w = kept, probs = 0.05),
            `95%` = Hmisc::wtd.quantile(lat, w = kept, probs = 0.95))

vtr_dat |>
  mutate(decade = 10*year%/%10) |>
  right_join(decade_bins, by = join_by(species_name, decade)) |>
  group_by(species_name, decade) |>
...
accum_maps <- lat_accum |>
  ...
  nest() |>
  mutate(map = map2(data, species_name, function(x,y){
    ggplot() + 
      geom_sf(data = usa) + geom_sf(data = can) + geom_sf(data = east_coast, fill = "transparent") +
      coord_sf(ylim = c(35,45), xlim = c(-66,-78)) + 
      geom_point(data = x, aes(x = lon, y = lat, color = partition, group = partition, alpha = kept)) +
