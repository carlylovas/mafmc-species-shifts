# Reading in MRIP Directed Trips data ----

library(survey)
library(tidyverse) 
library(here)

## Download & extract MRIP intercept data
base_url <- "https://www.st.nmfs.noaa.gov/st1/recreational/MRIP_Survey_Data/CSV/"
data_dir <- here("data", "MRIP_Index")
dir.create(data_dir, showWarnings = FALSE)

## Scrape directory listing for all ps_*.zip files and download
page <- readLines(base_url, warn = FALSE)
zip_names <- regmatches(page, gregexpr("ps_[^\"]+\\.zip", page))
zip_names <- unique(unlist(zip_names))
 
for (zn in zip_names) {
  destfile <- file.path(data_dir, zn)
  if (!file.exists(destfile)) {
    message("Downloading ", zn, "...")
    download.file(paste0(base_url, zn), destfile, mode = "wb")
  }
  message("Extracting ", zn, "...")
  unzip(destfile, exdir = data_dir)
}

## Organize all extracted CSVs into intYYYY folders based on filename
all_csvs <- list.files(data_dir, pattern = "\\.(csv|CSV)$", recursive = TRUE, full.names = TRUE)
for (f in all_csvs) {
...
# Load G.Nelson's MRIP.dirtrips.R ----
...
# Run ----
mid_atl   <- c(10, 24, 34, 36, 51) # Mid-Atlantic states
dom1      <- list(wave=list(c(1,2,3,4,5,6))) # All waves
